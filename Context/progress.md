...
`/admin/hot-spots` — five ordered published-venue pickers, "Save board"
replaces the whole `hot_spots` table (migration `0012_nostalgic_nova`:
`position` PK 1..5, `venue_id` FK, deny-all RLS). Write path:
`updateHotSpots` action → `requireAdmin` → `updateHotSpotsSchema` (≤5
distinct uuids) → published-venue check → `replaceHotSpots` (txn) →
`revalidateTag("hot-spots")`. Read: `getHotSpots`/`getHotSpotVenueIds`
(`unstable_cache` tag `hot-spots`); `page.tsx` SSR-loads it and threads
`hotSpotVenueIds` to `HotSpotsPanel`, which falls back to the
`HOT_SPOTS_THIS_WEEK` config slugs when the board is empty. Admin nav
gains a "Hot Spots" link.

Verified: `pnpm typecheck` / `lint` / `test` (185/185) / `build` clean;
`/admin/hot-spots` route builds; home Hot Spots tab confirmed rendering
the **config fallback** live against dev (which has no `hot_spots` table
yet — `getHotSpotVenueIds` throws, is caught, falls back). The admin
editor UI + DB path can't be exercised until migrations `0011` + `0012`
are applied and an admin signs in. **Not pushed** — migrate → deploy.
...
`feat/zone-overlay-cleanup-and-bulk-cuisine` fast-forward-merged into
`main` (no conflicts, no PR needed — clean linear history) and deployed
to `tueats.co`. Migrated and backfilled prod's database first: prod's
schema already matched migration `0010` (applied directly at some point,
outside `drizzle-kit migrate`), so the fix was baselining the journal's
two missing rows rather than replaying already-applied SQL — then ran
`pnpm backfill:map-zones` against prod, which had never received any of
today's zone-reorg work (still had `broad-st` and the pre-widening
`cecil-b-moore`/`avery` split). Post-backfill distribution matched dev
exactly. Verified live: homepage, `/eat/wendy-s`,
`/eat/hangry-joe-s-hot-chicken-and-beer` (both previously would have
crashed on the missing-zone-key bug), and a real Avery zone click all
confirmed working on production via Playwright. Full writeup, including
the credential note: `Context/decisions.md`.
...
The bulk cuisine editor (below) initially excluded "Cafe," reasoning it
was purely a `venue.type` value. Corrected per explicit site-owner
instruction: **Cafe exists as both a `venue.type` and a `CUISINES` tag**,
on purpose (PR #17 added it as a cuisine tag independent of `venue.type`;
a later same-day revert removed it again; this restores it a third time —
full history in `Context/decisions.md`). Re-added the one `CUISINES`
config entry; every consumer (bulk editor, admin multi-select, public
Cuisine filter, validation) already reads it dynamically, so nothing else
needed to change. The validation test that had asserted `cuisine: "cafe"`
is rejected now asserts it's accepted.

## 2026-09-04 — Remove visible zone overlays; admin bulk cuisine add/remove

Three independent changes, one branch (`feat/zone-overlay-cleanup-and-bulk-cuisine`, off `main` post-#22):

1. **Public map no longer draws zone geometry.** `MapZoneLayer` used to paint a colored wash + outline (`buildingFill` zones) and a dashed corridor line (`streetLine` zones) at the campus overview. Both are gone — only two layers remain: the invisible hit-fill (click-to-select) and the badge/label plate. `map-zones.ts`'s polygon data, `map-zones.geojson`, filtering (`pointInMapZone`), and the live "N spots" badge counts are all untouched — this was a pure render-layer removal, not a geometry or behavior change. `docs/design/map-and-pins.md`'s Zones section updated to match; `overlay-order.ts` dropped the four now-dead layer ids.
2. **Admin bulk cuisine editing.** The existing bulk-selection bar (until now Halal/Vegan Friendly Yes/No only) gained a cuisine picker + Add/Remove pair. New `bulkAddVenueCuisine`/`bulkRemoveVenueCuisine` queries use `array_append`/`array_remove` (guarded against duplicates) so only the one named tag changes — every other cuisine already on a row survives untouched. New `bulkSetCuisineSchema` + `bulkSetVenueCuisine` server action follow the same validate → `requireAdmin` → write → `revalidateTag` shape as the existing boolean bulk actions. "Cafe" was initially left out (mistaken for a `venue.type`-only concept) — **corrected below**, it's included like any other cuisine.
