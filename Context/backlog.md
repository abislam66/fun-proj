...
| Per-building hero tints / hover | Seed uses category palettes; individual landmark overrides (Student Center, Charles Library, Bell Tower) and `feature-state` hover are optional polish. | Visual QA pass on the split explorer, or when “near {building}” copy needs stronger map linkage. |
