...
import { getVenuePhotosBySlug } from "@/lib/db/queries";
 ...
 */
export async function VenuePhotoGallery({
  slug,
  venueName,
}: {
  slug: string;
  ...
  const photos = await getVenuePhotosBySlug(slug);
    ...
    <section aria-label={`Photos of ${venueName}`} className="detail-photos">
      <ul className="photo-strip">
        {photos.map((photo, index) => (
          <li className="photo-frame" key={photo.url}>
            <Image
              alt={photo.alt}
